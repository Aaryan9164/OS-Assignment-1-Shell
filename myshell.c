...
int parseInput(char* input_str, char** list)
{
...
char** command_args = get_commandArgs(list[0]);
...
{
// get the command and arg list from given command
char* dup_command = strdup(list[i]);
...
// allocate memory for the list of strings to store isolated
...
int ret = parseInput(str,list);

if(ret==1)
executeParallelCommands(list); // This function is invoked when user wants to run multiple commands in parallel (commands separated by &&)
else if(ret==2)
executeSequentialCommands(list); // This function is invoked when user wants to run multiple commands sequentially (commands separated by ##)
else if(ret==3)
executeCommandRedirection(list); // This function is invoked when user wants redirect output of a single command to and output file specificed by user
else
executeCommand(list); // This function is invoked when user wants to run a single commands
